...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initTrainer(settings = null, isTestMode = false) {
        ...
        function finalizePerformanceData() {
            ...
            // Counting accuracy (simplified for now)
            const countingAccuracy = 85; // Placeholder
            
            // Overall score (weighted average)
            const overallScore = (strategyAccuracy * 0.5) + (bettingAccuracy * 0.3) + (countingAccuracy * 0.2);
            
            // Average true count
            const avgTrueCount = performance.trueCountHistory.length > 0 ?
                performance.trueCountHistory.reduce((a, b) => a + b, 0) / performance.trueCountHistory.length : 0;
        ...
        function evaluateHands() {
            ...
            if (state.isTestMode && state.performance) {
                ...
                if (state.bettingEnabled) {
                    ...
                    const isCorrectBet = Math.abs(actualBet - recommendedBet) <= recommendedBet * 0.2; // 20% tolerance
